...
.section .isr_vector, "a", %progbits //a : section can be allocated,% : raw data, bits data no compiler touch it
.align 2 //thumb section sets so need 2 byte to align, ofc depend on the cpu, datasheet 
// interupt table an event table with adx needed for that event

.word _estack
.word Reset_Handler //first inst
...
.global Reset_Handler // this will be the x func, putting in interupt table, first inst of my os
...
1:	cmp r0,r1
	...
	strlt r3,[r0], #4
	blt 1b //b closest label named 1

	ldr r0, =_sbss
	ldr r1, =_ebss

	movs r2,#0 // initialized to 0 bss sections
